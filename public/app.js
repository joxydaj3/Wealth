...
window.goTo = function(pageId, btn) {
    ...
    // Carregamentos automáticos por página
    if(pageId === 'page-home') {
        window.loadUserData();
        window.loadHomeData(); 
    }
...
window.handleLogin = async function() {
    ...
    if(data.success) {
        ...
        else { await window.loadUserData(); window.goTo('page-home'); }
...
window.handleRegister = async function() {
    ...
    const res = await fetch('/api/register', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ phone, name, password: pass, invite: document.getElementById('r_invite').value }) });
    if(res.ok) { showAlert("Sucesso", "Conta criada!"); window.goTo('page-login'); }
...
window.doCheckin = async function() {
    ...
    const data = await res.json();
    if (res.ok) { showAlert("Bónus", `Ganhou MT ${data.amount}!`); window.loadUserData(); }
